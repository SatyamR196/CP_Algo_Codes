...
// Link : https://onecompiler.com/cpp/43mhxfmzy
//? Knapsack with restriction that if item i is taken then item i+1 can't be taken . 
//! Idea : 
//? 1) Method 1 : modify transition, if you take i, then clearly you can go to i+1,
//? so call dp(i+2) when you pick an item i.
//~ 1) Method 2 : modify state, add a bool parameter last_taken,
//~ when you taken item,call dp(i+1, last_taken =true), when you don't take,
//~ dp(i+1,last_taken=false) add check that take item only if is last_taken=false ;
//~ note is will add another dimentsion to cache array.
...
int dp(int level,int w_left){
    ...
    //choice 2 : take it in
    if(wi[level] <= w_left){
        ans = max(ans, vi[level] + dp(level+2, w_left - wi[level])) ;
    }
